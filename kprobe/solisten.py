...
int kprobe__inet_listen(struct pt_regs *ctx, struct socket *sock, int backlog)
{
        ...
        evt.recv_len = data_len;

        bpf_probe_read(evt.pkt, data_len, iodata);
        //unix_recv_events.perf_submit(ctx, evt, data_len+sizeof(u32));
        listen_evt.perf_submit(ctx, &evt, sizeof(evt));
...
if __name__ == "__main__":
    ...
    if args.show_netns:
        ...
        # print with data
        print("%-7s %-12s %-6s %-8s %-5s %-39s %-512s" %
              ("PID", "COMM", "PROTO", "BACKLOG", "PORT", "ADDR", "DATA"))
