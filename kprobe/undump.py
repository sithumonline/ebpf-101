...
def print_recv_pkg(cpu, data, size):
    ...
    #     pkt_str = binascii.hexlify(struct.pack("B", event.pkt[i])).decode("utf-8")
    #     print("%s, " % pkt_str, end="")
    #     sys.stdout.flush()
    #     if (i+1)%16 == 0:
    #         print("")
    ...
    pkt_str = binascii.hexlify(event.pkt).decode("utf-8")
    print("%s" % pkt_str)
    ...
    # sleep 5 seconds to avoid too many output
    sleep(5)
